								...
								<form action="">
									...
									<div class="inline">
										<select name="sort" class="form-control">
											...
											<option value="priceAsc">Prices go up</option>
											<option value="priceDes">Prices go down</option>
										</select>
									</div>
									<div class="inline">
										<input class="search-submit" type="submit" value="Rearrange">
									</div>
								</form>
							</div>
						</div>
						...
						<c:forEach var="sanpham" items="${list}" varStatus="loop">
							<c:if test="${loop.index != list.size()-1}">
								<c:if test="${loop.index%4 == 0}">
									<div class="section group">
										<div class="grid_1_of_4 images_1_of_4 products-info"><a
												href="sp?id=${sanpham.id}">
												<img style="width: 300px; height: 238px"
											...
											<button onClick="addToCart(${sanpham.id})" class="btn cart-btn"><span
													class="glyphicon glyphicon-shopping-cart pull-center"></span>
												Cart</button>
											<h3></h3>
										</div>
								</c:if>
				...
				<c:if test="${list.size() != 0}">
					<div class="paging">
						<c:if test="${currentPage != 1}">
							<a
								href="/haonguyenshop/search?name=${name}&page=${currentPage-1}&sort=${sort}&range=${range}&brand=${brand}&manufactor=${manufactor}">Back</a>
						</c:if>
						<c:if test="${currentPage == 1}">
							<a class="current">1</a>
						</c:if>
						<c:if test="${currentPage != 1}">
							<a
								href="/haonguyenshop/search?name=${name}&page=1&sort=${sort}&range=${range}&brand=${brand}&manufactor=${manufactor}">1</a>
						</c:if>
						...
						<c:forEach var="pag" items="${pageList}" varStatus="loop">
							...
							<c:if test="${currentPage != pag}">
								<a
									href="/haonguyenshop/search?name=${name}&page=${pag}&sort=${sort}&range=${range}&brand=${brand}&manufactor=${manufactor}">${pag}</a>
							</c:if>
						</c:forEach>

						<c:if test="${currentPage != totalPage}">
							<a
								href="/haonguyenshop/search?name=${name}&page=${currentPage+1}&sort=${sort}&range=${range}&brand=${brand}&manufactor=${manufactor}">Next</a>
						</c:if>
