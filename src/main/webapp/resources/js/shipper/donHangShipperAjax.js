$(document).ready(function() {
	...
	function ajaxGet(page){		
		...
		$.ajax({
			...
			url: "http://localhost:8080/haonguyenshop/api/shipper/don-hang/all" + '?page=' + page,
			success: function(result){
				$.each(result.content, function(i, donHang){
					// tính giá trị đơn hàng
					var sum = 0;
					if(donHang.trangThaiDonHang == "complete" || donHang.trangThaiDonHang == "Waiting for approval" ){
					...
					var donHangRow = '<tr>' +
					     ...
					     if(donHang.trangThaiDonHang == "assigned"){
...
//		event.preventDefault();
	...
	$(document).on('keyup', '#searchById', function (event){
		...
		if(donHangId != ''){
    	  $('.donHangTable tbody tr').remove();
    	  $('.pagination li').remove();
		  var href = "http://localhost:8080/haonguyenshop/api/shipper/don-hang/"+donHangId;
...
//		console.log(donHangId);
		...
		$.get(href, function(donHang) {
			...
			$('#trangThaiDonHang').text("Order Status: "+ donHang.trangThaiDonHang);
			...
			if(donHang.ghiChu != null){
				$("#ghiChu").text("Note: "+ donHang.ghiChu);
			}
			
			if(donHang.nguoiDat != null){
				$("#nguoiDat").text("Orderer: "+ donHang.nguoiDat.hoTen);
			}
			...
			if(check){
				$('.chiTietTable').find('thead tr').append('<th id="soLuongNhanTag" class="border-0 text-uppercase small font-weight-bold">Received quantity</th>');
			}
			// thêm bảng:
			var sum = 0;
			var stt = 1;
	...
	// post request Update trạng thái đơn shipper
...
//    	 console.log(data);
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8080/haonguyenshop/api/shipper/don-hang/update",
