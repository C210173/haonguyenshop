$(document).ready(function(){
	...
	$('.btnCapNhatThongTin').on("click", function(event) {
		...
		var href = "http://localhost:8080/haonguyenshop/api/profile/"+nguoiDungId;
	...
	function ajaxPostChangePass() {
	    	// PREPATEE DATA
	    	 ...
	    	 $.ajax({
	 			...
	 			url : "http://localhost:8080/haonguyenshop/api/profile/doiMatKhau",
	 			data : JSON.stringify(data),
	 			success : function(response) {
	 				if(response.status == "success"){
	 					$('#doiMKModal').modal('hide');
	 					alert("Change password successfully. You must log in again to confirm");
	 					location.href = "http://localhost:8080/haonguyenshop/logout";
	 				} else {
	 			    	$('input').next().remove();
