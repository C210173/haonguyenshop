$(document).ready(function(){
	...
	function ajaxGet(page){
		$.ajax({
			type: "GET",		
			url: "http://localhost:8080/haonguyenshop/api/nhan-hieu/all?page=" + page,
				...
				$.each(result.content, function(i, nhanHieu){
					var nhanHieuRow = '<tr style="text-align: center;">' +
					                  ...
					                  '<td>'+'<input type="hidden" value=' + nhanHieu.id + '>'
					                     + '<button class="btn btn-primary btnCapNhatNhanHieu" >Update</button>' +
					                     '   <button class="btn btn-danger btnXoaNhanHieu">Delete</button></td>'
				                      '</tr>';
	...
	function ajaxPost(){
    	...
    	$.ajax({
			...
			url : "http://localhost:8080/haonguyenshop/api/nhan-hieu/save",
    ...
    $(document).on("click",".btnCapNhatNhanHieu",function(){
		...
		var href = "http://localhost:8080/haonguyenshop/api/nhan-hieu/" + nhanHieuId;
    ...
    function ajaxPut(){
    	...
    	$.ajax({
			...
			type : "PUT",
			contentType : "application/json",
			url : "http://localhost:8080/haonguyenshop/api/nhan-hieu/update",
			data : JSON.stringify(formData),
            // dataType : 'json',
			success : function(response) {
    ...
    $(document).on("click",".btnXoaNhanHieu", function() {
		...
		var confirmation = confirm("Are you sure delete this brand?");
		if(confirmation){
		  $.ajax({
			  type : "DELETE",
			  url : "http://localhost:8080/haonguyenshop/api/nhan-hieu/delete/" + nhanHieuId,
			  success: function(resultMsg){
				 resetDataForDelete();
				 alert("Delete successful");
			  },
			  error : function(e) {
				 alert("This brand cannot be deleted, please check again");
				 console.log("ERROR: ", e);
			  }
		  });
		}
