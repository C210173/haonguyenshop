$(document).ready(function() {
	...
	function ajaxGet(page){
		...
		$.ajax({
			...
			url: "http://localhost:8080/haonguyenshop/api/san-pham/all" + '?page=' + page,
			success: function(result){
				$.each(result.content, function(i, sanPham){
					var sanPhamRow = '<tr>' +
					                  '<td>' + '<img src="/haonguyenshop/img/'+sanPham.id+'.png" class="img-responsive" style="height: 50px; width: 50px" />'+'</td>' +
					...
					sanPhamRow += ( checkTenDanhMuc != -1)?'<button class="btn btn-primary btnCapNhatLapTop" >Update</button>':'<button class="btn btn-primary btnCapNhatOther" >Update</button>';
					sanPhamRow += '  <button class="btn btn-danger btnXoaSanPham">Delete</button></td>'+'</tr>';
	...
	$('#danhMucDropdown').mouseup(function() {
		...
		if (open) {
            ...
            $(".modal-title").text("Add new product category "+ label);
    ...
    function ajaxPostLapTop() {
    	// PREPATEE DATA
    	 ...
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8080/haonguyenshop/api/san-pham/save",
    ...
    $(document).on("click",".btnCapNhatLapTop", function(event){
		...
		var href = "http://localhost:8080/haonguyenshop/api/san-pham/"+sanPhamId;
    ...
    $(document).on("click",".btnXoaSanPham", function() {
		...
		var confirmation = confirm("Are you sure Delete this product?");
		if(confirmation){
		  $.ajax({
			  async:false,
			  type : "DELETE",
			  url : "http://localhost:8080/haonguyenshop/api/san-pham/delete/" + sanPhamId,
    ...
    $(document).on("click",".btnChiTiet", function() {
    	...
    	var href = "http://localhost:8080/haonguyenshop/api/san-pham/"+sanPhamId;
		$.get(href, function(sanPham) {
			$('.hinhAnh').attr("src", "/haonguyenshop/img/"+sanPham.id+".png");
			$('.tenSanPham').html("<span style='font-weight: bold'>Product name: </span> "+ sanPham.tenSanPham);
			$('.maSanPham').html("<span style='font-weight: bold'>Product code: </span>"+ sanPham.id);
			$('.hangSangXuat').html("<span style='font-weight: bold'>Manufacturer: </span>"+ sanPham.hangSanXuat.tenHangSanXuat);
			...
			if(checkTenDanhMuc != -1){
			  ...
			  $('.thietKe').html("<span style='font-weight: bold'>Design: </span>"+ sanPham.thietKe);
			  $('.dungLuongPin').html("<span style='font-weight: bold'>Battery: </span>"+ sanPham.dungLuongPin);
			  $('.heDieuHanh').html("<span style='font-weight: bold'>OS: </span>"+ sanPham.heDieuHanh);
			  $('.manHinh').html("<span style='font-weight: bold'>Display: </span>"+ sanPham.manHinh);
			}
			$('.thongTinChung').html("<span style='font-weight: bold'>General information: </span>"+ sanPham.thongTinChung);
			$('.donGia').html("<span style='font-weight: bold'>Price: </span>"+ sanPham.donGia +" $");
			$('.baoHanh').html("<span style='font-weight: bold'>Guarantee: </span>"+ sanPham.thongTinBaoHanh);
			$('.donViKho').html("<span style='font-weight: bold'>Products in stock: </span>"+ sanPham.donViKho);
			$('.donViBan').html("<span style='font-weight: bold'>products sold: </span>"+ sanPham.donViBan);
